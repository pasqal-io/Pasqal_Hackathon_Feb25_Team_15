...
def train_and_evaluate_qek_model(X_train_scaled, X_test_scaled, y_train, y_test, feature_names=None):
    ...
    Uses the kernel matrix approach shown in the QEK tutorial.
    ...
    """
    print("\n----- Quantum Evolution Kernel (QEK) Model -----")
    ...
    try:
        ...
        # More efficient conversion to state dictionaries
        def feature_to_state_dict(features):
            """
            Convert feature vector to a quantum state dictionary more efficiently.
            Limits the number of states to keep computation manageable.
            """
            # Normalize features for valid probability distribution
            ...
            # Create state dictionary - only use top K largest probabilities
            state_dict = {}
            K = min(8, len(features))  # Limit number of states to reduce computation
            
            # Get indices of top K probabilities
            top_indices = np.argsort(probs)[-K:]
            
            # Create binary strings only for top K states
            for i, idx in enumerate(top_indices):
                binary = format(i, f'0{3}b')  # Use fixed 3-bit representation
                if probs[idx] > 1e-10:
                    state_dict[binary] = float(probs[idx])
        ...
        # Create smaller, more manageable datasets
        print("Creating optimized QEK-compatible data objects...")
        # Reduce dataset size if necessary to prevent stalling
        max_samples = min(1000, len(X_train_scaled_clean))  # Limit to max 1000 samples
        
        # Use stratified sampling to maintain class balance
        from sklearn.model_selection import StratifiedShuffleSplit
        
        if len(X_train_scaled_clean) > max_samples:
            print(f"Reducing training data from {len(X_train_scaled_clean)} to {max_samples} samples for QEK")
        ...
        # Similarly, limit test set size if needed
        max_test_samples = min(200, len(X_test_scaled_clean))
        if len(X_test_scaled_clean) > max_test_samples:
            print(f"Reducing test data from {len(X_test_scaled_clean)} to {max_test_samples} samples for QEK")
        ...
        print(f"Using {len(X_train_reduced)} training samples and {len(X_test_reduced)} test samples for QEK")
            
        # Convert data to QEK format with optimized state dictionaries
        train_data = [
        ...
        # Verify objects have the required attributes
        print(f"Verifying data objects format: {type(train_data[0])}")
        print(f"First object has state_dict: {hasattr(train_data[0], 'state_dict')}")
        print(f"First object has target: {hasattr(train_data[0], 'target')}")
        
        # Print first state dict to verify structure
        print(f"Sample state_dict format: {train_data[0].state_dict}")
        
        if not hasattr(train_data[0], 'state_dict'):
            raise AttributeError("Data objects missing required 'state_dict' attribute")
        
        # Initialize QEK with smaller mu value for faster computation
        print("Initializing QEK with μ=0.1")
        start_time = time.time()
        kernel = QEK(mu=1.0)  # Lower mu value for faster computation
        
        # Use optimized kernel computation in batches
        print("Computing kernel matrices in batches...")
        batch_size = 50  # Small batch size for better progress tracking
        ...
        print("Training SVM with precomputed kernel matrix...")
        from sklearn.svm import SVC
        ...
        model = SVC(
            ...
            random_state=42,
            class_weight='balanced',
            C=100.0  # Start with default C 1.0 best 1.0
        )
